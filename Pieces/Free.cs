...
namespace Xadrez_TIC.Pieces
{
    class Free : Piece
    {
        ...
        public Free(Board tab, Position pos, string pType, Color color = Color.White) : base(tab, color)
        {
            this.tab = tab;
            position = pos;
        ...
        public Free(Board tab, string pType, Color color = Color.White) : base(tab, color)
        {
            this.tab = tab;
            if (pType == "livre" || pType == "ponto") { pieceType = pType; }
            else { throw new FatalException("Erro no tipo da peça aplicada ao tipo Free"); }
